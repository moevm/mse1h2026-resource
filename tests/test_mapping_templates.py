...
def test_builtin_mapping_templates_are_available():
    templates = mapping_template_repo.list()
    ids = {template.id for template in templates}

    assert "watcher-kubernetes-objects-v1" in ids
    assert "watcher-otel-traces-v1" in ids
    assert "watcher-prometheus-scrape-v1" in ids
...
def test_otel_template_maps_calls_and_data_dependencies(monkeypatch):
    payloads = [
        {
            "resourceSpans": [
                {
                    "resource": {
                        "attributes": [
                            {"key": "service.name", "value": {"stringValue": "fastapi-app"}},
                            ...
                            "spans": [
                                {
                                    ...
                                    "attributes": [
                                        ...
                                        {"key": "db.name", "value": {"stringValue": "postgres-db"}},
            ...
            "resourceSpans": [
                {
                    "resource": {
                        "attributes": [
                            {"key": "service.name", "value": {"stringValue": "flask-app"}},
                            {"key": "telemetry.sdk.name", "value": {"stringValue": "opentelemetry"}},
                            {"key": "telemetry.sdk.language", "value": {"stringValue": "python"}},
                        ]
                    },
                            ...
                            "spans": [
                                {
                                    "name": "HTTP GET",
                                    "attributes": [
                                        {"key": "peer.service", "value": {"stringValue": "fastapi-app"}},
    ...
    mapping, nodes = _map_payloads("watcher-otel-traces-v1", payloads)
    node_types = {node["type"] for node in nodes}

    assert {"Service", "Endpoint", "Database", "Table", "Library"} <= node_types

    edges, unresolved = _edge_tuples(nodes, mapping, monkeypatch)

    assert ("urn:service:flask-app", "urn:service:fastapi-app", "calls") in edges
    assert ("urn:service:fastapi-app", "urn:database:postgres-db", "reads") in edges
    assert ("urn:service:fastapi-app", "urn:table:Users", "reads") in edges
    assert ("urn:table:Users", "urn:database:postgres-db", "ownedby") in edges
    assert ("urn:endpoint:GET /users", "urn:service:fastapi-app", "ownedby") in edges
    assert ("urn:service:fastapi-app", "urn:library:opentelemetry", "dependson") in edges
    assert not unresolved


def test_prometheus_template_maps_service_runtime_payload():
    payloads = [
        {
            "labels": {
                "service": "flask-app",
    ...
    _, nodes = _map_payloads("watcher-prometheus-scrape-v1", payloads)

    assert len(nodes) == 1
    assert nodes[0]["type"] == "Service"
    assert nodes[0]["id"] == "urn:service:flask-app"
    assert nodes[0]["memory_allocated_mb"] == 100
