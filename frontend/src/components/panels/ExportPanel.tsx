...
import { downloadExport, fetchExportFormats } from '../../api/graphApi';
...
export default function ExportPanel() {
  ...
  const { nodeTypes, edgeTypes } = useMemo(() => {
    ...
    return {
      nodeTypes: hiddenNodeTypes.size > 0 && visibleNodes.length !== allNodeTypes.length ? visibleNodes : [],
      edgeTypes: hiddenEdgeTypes.size > 0 && visibleEdges.length !== allEdgeTypes.length ? visibleEdges : [],
    };
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      const ext = formats.find((f) => f.format === selectedFormat)?.extension ?? `.${selectedFormat}`;
  ...
  return (
    <div className="flex flex-col gap-4 p-4 text-sm">
      ...
      <p className="rounded-md bg-slate-800/40 px-2.5 py-2 text-xs text-slate-400">{filterSummary}</p>
