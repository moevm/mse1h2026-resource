...
function DraggableField({
  ...
  const bgColor = isMapped ? 'bg-emerald-500/20 border-b border-emerald-500/50' : 'hover:bg-blue-500/20';

  return (
    <span
      ref={ref}
      className={`cursor-grab active:cursor-grabbing ${bgColor} rounded px-1 transition-colors ${
        isDragging ? 'bg-blue-500/30 opacity-50' : ''
      }`}
...
export function RawDataPanel({ data, chunkId, onCreateMapping, fieldMappings = [] }: RawDataPanelProps) {
  ...
  const renderValue = (value: unknown, path: string, depth: number = 0): React.ReactNode => {
      ...
      return (
        <div>
          <span
            role="button"
            tabIndex={0}
            className="cursor-pointer text-slate-400 select-none hover:text-slate-300"
            onClick={() => toggleJsonPath(path)}
            onKeyDown={(e) => {
              if (e.key === 'Enter' || e.key === ' ') toggleJsonPath(path);
            }}
          >
            {isExpanded ? '▼' : '▶'} <span className="text-slate-500">[{count}]</span>
          </span>
          {isExpanded && (
            <div className="mt-1 ml-2 border-l-2 border-slate-700 pl-2">
              ...
              {entries.map(([key, val]) => {
                ...
                return (
                  <div key={key} style={{ marginLeft: indent }} className="py-0.5">
                    <DraggableField path={fieldPath} value={val} dataType={getDataType(val)} isMapped={fieldIsMapped}>
                      <span className="font-medium text-amber-400 hover:text-amber-300">{key}:</span>
                    </DraggableField>{' '}
    ...
    return <span className="text-slate-400">{typeof value === 'string' ? value : JSON.stringify(value)}</span>;
  };

  const renderNavigationItem = (key: string, value: unknown, path: string): React.ReactNode => {
    const dataType = getDataType(value);
    ...
    return (
      <div
        ...
        className={`group flex cursor-pointer items-center gap-2 rounded border px-2 py-1.5 hover:bg-slate-700/30 ${
        ...
        onKeyDown={
          isObject
            ? (e) => {
                if (e.key === 'Enter' || e.key === ' ') navigateInto(key);
              }
            : undefined
        }
      >
        <DraggableField path={path} value={value} dataType={dataType} isMapped={isMapped}>
          <span
            className={`font-medium group-hover:text-amber-300 ${isMapped ? 'text-emerald-400' : 'text-amber-400'}`}
          >
            {key}
          </span>
        </DraggableField>
        ...
        {dataType === 'object' && (
          <>
            ...
            <span className="text-xs text-slate-600">{nestedCount}</span>
            <span className="ml-auto text-xs text-blue-400 opacity-0 group-hover:opacity-100">→ enter</span>
          </>
        )}
        {dataType === 'array' && (
          <>
        ...
        {dataType === 'string' && typeof value === 'string' && (
          <>
            <span className="max-w-[150px] truncate text-xs text-green-400">
        ...
        {dataType === 'boolean' && (
          <>
            <span className="text-xs text-purple-400">{value ? 'true' : 'false'}</span>
            {isMapped && (
              <span className="ml-auto text-xs text-emerald-400">
                → {mappingInfo.mapping.target_node_type}.{mappingInfo.mapping.target_field}
              </span>
        ...
        {dataType === 'null' && (
          <>
            <span className="text-xs text-slate-500 italic">null</span>
            {isMapped && (
              <span className="ml-auto text-xs text-emerald-400">
                → {mappingInfo.mapping.target_node_type}.{mappingInfo.mapping.target_field}
              </span>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex shrink-0 items-center gap-2 border-b border-slate-700/50 bg-slate-800/30 px-3 py-2">
        <span className="text-xs text-slate-500">Drag fields to schema →</span>
        <div className="flex-1" />
        {chunkId && onCreateMapping && (
          <button
            onClick={onCreateMapping}
            className="rounded bg-emerald-600/80 px-2 py-0.5 text-xs text-white hover:bg-emerald-700"
          >
            + Create Mapping
          </button>
        )}
        <button
          ...
          className="rounded px-2 py-0.5 text-xs text-slate-500 hover:bg-slate-700/50 hover:text-slate-400"
        >
          + Expand All
        </button>
        <button
      ...
      {navigationPath && (
        <div className="flex shrink-0 items-center gap-1 overflow-x-auto border-b border-slate-700/50 bg-slate-800/50 px-3 py-2 text-xs">
          <button onClick={() => setNavigationPath('')} className="text-blue-400 hover:text-blue-300">
            root
          </button>
          ...
          <button
            onClick={navigateUp}
            className="ml-2 rounded bg-slate-700/50 px-1.5 py-0.5 text-slate-500 hover:text-slate-400"
          >
            ↑ Up
          </button>
        </div>
      ...
      <div className="flex-1 overflow-auto">
        {navigationPath ? (
          <div className="space-y-0.5 p-2">
            ...
            {currentEntries.length === 0 && <div className="py-4 text-center text-sm text-slate-500">Empty object</div>}
          </div>
        ) : (
          <div className="p-3 font-mono text-sm">{renderValue(data, '', 0)}</div>
        )}
      </div>

      <div className="flex shrink-0 items-center gap-4 border-t border-slate-700/50 bg-slate-800/30 px-3 py-1.5 text-xs text-slate-500">
        <span>
          {navigationPath ? (
            <>
