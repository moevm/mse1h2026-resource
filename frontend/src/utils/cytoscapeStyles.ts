...
import { getEdgeColor, getNodeColor, getNodeShape, getStatusColor } from './colors';
...
function buildNodeTypeStyles(hiddenTypes: Set<string>, filterMode: 'ghost' | 'exclude'): Stylesheet[] {
  const allTypes = [
    'Service',
    'Endpoint',
    'Deployment',
  ...
  return allTypes.map((type) => {
    ...
    return {
      ...
      style: {
        ...
        ...(visible ? { opacity: 1 } : filterMode === 'exclude' ? { display: 'none' } : { opacity: 0.06 }),
      },
    };
  });
}
...
function buildEdgeTypeStyles(hiddenTypes: Set<string>, filterMode: 'ghost' | 'exclude'): Stylesheet[] {
  const allTypes = [
    'calls',
    'publishesto',
    'consumesfrom',
  ...
  return allTypes.map((type) => {
    ...
    return {
      ...
      style: {
        ...
        ...(visible ? { opacity: 0.7 } : filterMode === 'exclude' ? { display: 'none' } : { opacity: 0.04 }),
      },
    };
  });
}
