...
export function findShortestPathLocal(
  ...
  const adjacency = buildAdjacency(edges, options.direction ?? 'directed', options.allowedEdgeTypes);

  return bfsShortestPath(sourceId, targetId, adjacency, options.maxDepth ?? Number.POSITIVE_INFINITY);
}
...
function bfsShortestPath(
  ...
  while (queue.length > 0) {
    ...
    const found = visitNeighbors(current, currentDepth, neighbors, targetId, queue, depthByNode, prevByNode);
