...
export interface LogState {
  ...
  addLog: (level: LogLevel, source: string, message: string, details?: Record<string, unknown>) => void;
  clearLogs: () => void;
...
function createLogEntry(level: LogLevel, source: string, message: string, details?: Record<string, unknown>): LogEntry {
  _logCounter += 1;
  return {
...
export const useLogStore = create<LogState>((set, get) => ({
  ...
  addEntry: (entry) =>
    set((s) => {
      const newEntry = createLogEntry(entry.level, entry.source, entry.message, entry.details);
